...
const ChangeProfile = ({ onClose, currentName, currentBio }) => {
  ...
  const validateForm = () => {
    ...
    // At least one field should be different from current values
    const nameChanged = formData.name.trim() !== (currentName || '');
    const bioChanged = formData.bio.trim() !== (currentBio || '');
    
    if (!nameChanged && !bioChanged) {
      newErrors.submit = 'Please make at least one change to update your profile';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const variables = {};
      if (formData.name.trim()) {
        variables.name = formData.name.trim();
      }
      if (formData.bio.trim()) {
        variables.bio = formData.bio.trim();
      }
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <div className="absolute left-3 top-3">
                <FaEdit className="text-gray-400" size={16} />
              </div>
              <textarea
                name="bio"
                value={formData.bio}
